...
def loadMcCsv(fileName, dbName, symbol):
    ...
    for d in reader:
        ...
        bar.openInterest = float(d['TotalPosition'])   # 持仓量
...
if __name__ == '__main__':
    ...
    #loadMcCsv('C:/Users/LZJF_02/Desktop/IF0000_1min.csv', 'Data', 'IF0000')
    #DataBase_name = 'Data'
    data_path = 'C:/Users/LZJF_02/Desktop/modified_data/'
    filename = 'ru000_1min_modi.csv'
    future_name = 'ru000_1min'
    #loadMcCsv(data_path + filename, DataBase_name, future_name)

    #导入通达信历史分钟数据
    #loadTdxCsv('CL8.csv', MINUTE_DB_NAME, 'c0000')
    dataBasesprefix = 'FutureData'
    for csvfile in  os.listdir(data_path):
        dataname = csvfile.split('.')[0]
        if '000' in dataname:
            appendix = 'Index'
        elif '888' in dataname:
            appendix = 'Sequence'
        dataBaseName = dataBasesprefix + '_' + appendix
        #if csvfile == 'rb000_1min_modi.csv':
         #   print dataname + ' already in ' + dataBaseName + ' !'
          #  continue

        #loadMcCsv(data_path + csvfile, dataBaseName, dataname)
        print dataname+ ' of ' +csvfile + ' in ' + dataBaseName


        #loadMcCsv(data_path + csvfile, DataBase_name, future_name)
